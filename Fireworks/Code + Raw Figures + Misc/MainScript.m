...
dv = 3;
parents = 12;
Lambda = [rand(S, 1),rand(S, 1)*4+1,rand(S, 1)*9+1];
PI = zeros(G, S);
Orig = zeros(G, S);
Fitness = ones(S, 1);
Generation = 1;
...
for k = 1:S
    Fitness(k) = CostFunction(Lambda(k,1),Lambda(k,2),Lambda(k,3));
end
[SortedOrder, IndexArray] = sort(Fitness);
PI(Generation, :) = SortedOrder;
Orig(Generation, :) = IndexArray;
while Generation < G
    for i = 1:parents/2
        theStrings(2*i-1, :) = Lambda(Orig(Generation, 2*i-1), :);
        theStrings(2*i, :) = Lambda(Orig(Generation, 2*i), :);
        for j = 1:2
            randomPhi = rand(1,dv)*2 - 0.5;
            OSiteration(j, :) = theStrings(2*i-1, :).*randomPhi + theStrings(2*i, :).*(1-randomPhi);
        end
        OSoverall(2*i-1:2*i, :) = OSiteration;
    end
    theStrings(parents+1:2*parents, :) = OSoverall;
    theStrings(2*parents+1:S, :) = [rand(S-(2*parents), 1),rand(S-(2*parents), 1)*4+1,rand(S-(2*parents), 1)*9+1];
    theStrings(theStrings<0) = 0;
    Lambda = theStrings; 
    ...
    PI(Generation, :) = SortedOrder;
    Orig(Generation, :) = IndexArray;
    GenerationCost(i) = nanmean(Fitness);
    ParentCost(i) = nanmean(Fitness(1:parents));
    BestCost(i) = SortedOrder(1);
    fprintf("Generation %d completed, best cost so far is %d", Generation, PI(Generation, 1))
    fprintf("\n")
end
figure
hold on
...
%         end
%         theStrings(parents+1:2*parents, :) = OSoverall;
%         theStrings(2*parents+1:S, :) = 40*rand(S-(2*parents), dv) - 20;
%         Lambda = theStrings;       
%     end
%     Generation = Generation + 1; 
...
function Cost = CostFunction(m_e, m_f, t_e)
global N_p
global sphere_bois
...
Cost45 = w4 * (max(interpolated_temp) - theta_goal)^2 + w5 * (m_e - m__max);
Cost = Cost123 + Cost45;
end
